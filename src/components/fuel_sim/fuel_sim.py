...
class FuelSim:
    class Fuel:
        ...
        def update(
            ...
            self._handle_field_collisions(
                subticks, blue_hub, red_hub, relevant_lines, relevant_trenches
            )

        def _handle_xz_line_collision(
            self, line_start: Translation3d, line_end: Translation3d
        ) -> None:
            """
            ...
            self.vel = self.vel - (
                normal * ((1 + const.FIELD_COR) * self.vel.dot(normal))
            )
        ...
        def _handle_field_collisions(
            ...
            if relevant_lines is not None:
                ...
                for i in range(len(const.FIELD_XZ_LINE_STARTS)):
                    self._handle_xz_line_collision(
                        const.FIELD_XZ_LINE_STARTS[i], const.FIELD_XZ_LINE_ENDS[i]
                    )

            # edges - X axis
            if self.pos.X() < const.FUEL_RADIUS and self.vel.X() < 0:
                self.pos = self.pos + Translation3d(
                    const.FUEL_RADIUS - self.pos.X(), 0, 0
                )
                self.vel = self.vel + Translation3d(
                    -(1 + const.FIELD_COR) * self.vel.X(), 0, 0
                )
            elif (
                self.pos.X() > const.FIELD_LENGTH - const.FUEL_RADIUS
                and self.vel.X() > 0
            ):
                self.pos = self.pos + Translation3d(
                    const.FIELD_LENGTH - const.FUEL_RADIUS - self.pos.X(), 0, 0
                )
                self.vel = self.vel + Translation3d(
                    -(1 + const.FIELD_COR) * self.vel.X(), 0, 0
                )
            ...
            if self.pos.Y() < const.FUEL_RADIUS and self.vel.Y() < 0:
                self.pos = self.pos + Translation3d(
                    0, const.FUEL_RADIUS - self.pos.Y(), 0
                )
                self.vel = self.vel + Translation3d(
                    0, -(1 + const.FIELD_COR) * self.vel.Y(), 0
                )
            elif (
                self.pos.Y() > const.FIELD_WIDTH - const.FUEL_RADIUS
                and self.vel.Y() > 0
            ):
                self.pos = self.pos + Translation3d(
                    0, const.FIELD_WIDTH - const.FUEL_RADIUS - self.pos.Y(), 0
                )
                self.vel = self.vel + Translation3d(
                    0, -(1 + const.FIELD_COR) * self.vel.Y(), 0
                )
    ...
    @staticmethod
    def _fuel_collide_rectangle(
        fuel: "FuelSim.Fuel", start: Translation3d, end: Translation3d
    ):
        """
        Handle collision between fuel and a rectangular obstacle. Optimized to avoid sqrt calls.
        """
        if (
            fuel.pos.Z() > end.Z() + const.FUEL_RADIUS
            or fuel.pos.Z() < start.Z() - const.FUEL_RADIUS
        ):
        ...
        if (
            distance_to_left > 0
            or distance_to_right > 0
            or distance_to_top > 0
        ...
        if max_dist == distance_to_left:
            ...
            fuel.vel = fuel.vel + Translation3d(
                -(1 + const.FIELD_COR) * fuel.vel.X(), 0, 0
            )
            ...
            fuel.vel = fuel.vel + Translation3d(
                0, -(1 + const.FIELD_COR) * fuel.vel.Y(), 0
            )
    ...
    TRENCH_RECTANGLES = [
        (
            ...
            Translation3d(
                5.18, const.TRENCH_WIDTH + const.TRENCH_BLOCK_WIDTH, const.TRENCH_HEIGHT
            ),
        ),
        (
            ...
            Translation3d(
                5.18,
                const.FIELD_WIDTH - 1.57 + const.TRENCH_BLOCK_WIDTH,
                const.TRENCH_HEIGHT,
            ),
        ),
        (
    ...
    def spawn_starting_fuel(self) -> None:
        ...
        center = Translation3d(
            const.FIELD_LENGTH / 2, const.FIELD_WIDTH / 2, const.FUEL_RADIUS
        )
        for i in range(15):
            for j in range(6):
                self.fuels.append(
                    FuelSim.Fuel(
                        center
                        + Translation3d(
                            0.076 + 0.152 * j, 0.0254 + 0.076 + 0.152 * i, 0
                        )
                    )
                )
                self.fuels.append(
        ...
        for i in range(3):
            for j in range(4):
                ...
                self.fuels.append(
                    FuelSim.Fuel(
                        Translation3d(
                            const.FIELD_LENGTH - 0.076 - 0.152 * j,
                            2.09 + 0.076 + 0.152 * i,
                            const.FUEL_RADIUS,
                        )
                    )
    ...
    def register_robot(
        ...
        field_speeds_supplier: Callable[[], ChassisVelocities],
    ) -> None:
        """
        ...
        Args:
            ...
            field_speeds_supplier: callable that returns field-relative ChassisVelocities
        """
    ...
    def launch_fuel(
        ...
        Args:
            ...
            hood_angle: Hood angle where 0 is horizontal and π/2 (pi/2) is straight up (in radians)
            shooter_yaw: Robot-relative shooter yaw (in radians)
    ...
    def _handle_robot_collision(
        self, fuel: "FuelSim.Fuel", robot: Pose2d, robot_vel: Translation2d
    ) -> None:
        """
        Handles collision between fuel and robot.
        """
        relative_pos = (
            Pose2d(fuel.pos.toTranslation2d(), Rotation2d())
            .relativeTo(robot)
            .translation()
        )
        ...
        distance_to_bottom = (
            -const.FUEL_RADIUS - self.robot_length / 2 - relative_pos.X()
        )
        ...
        if (
            ...
            distance_to_right >= distance_to_bottom
        ...
        if fuel.vel.toTranslation2d().dot(normal) < 0:
            fuel.add_impulse(
                Translation3d(
                    normal
                    * (-fuel.vel.toTranslation2d().dot(normal) * (1 + const.ROBOT_COR))
                )
            )
    ...
    def _handle_robot_collisions(self, fuels: list["FuelSim.Fuel"]) -> None:
        ...
        robot_interaction_radius = (
            self.robot_width / 2 + self.robot_length / 2
        ) * 1.5 + const.FUEL_RADIUS
    ...
    def register_intake(
        ...
        self.intakes.append(
            FuelSim.SimIntake(
                x_min, x_max, y_min, y_max, able_to_intake, intake_callback
            )
        )
    ...
    class Hub:
        ...
        def __init__(
            self, center: Translation2d, exit_pos: Translation3d, exit_vel_x_mult: int
        ):
            self.center = center
            self.exit = exit_pos
        ...
        def did_fuel_score(self, fuel: "FuelSim.Fuel", subticks: int) -> bool:
            ...
            """
            return (
                fuel.pos.toTranslation2d().distance(self.center)
                <= const.HUB_ENTRY_RADIUS
                and fuel.pos.Z() <= const.HUB_ENTRY_HEIGHT
                and (fuel.pos - (fuel.vel * (const.PERIOD / subticks))).Z()
                > const.HUB_ENTRY_HEIGHT
            )
        ...
        def fuel_collide_side(self, fuel: "FuelSim.Fuel") -> None:
            ...
            FuelSim._fuel_collide_rectangle(
                fuel,
                Translation3d(
                    self.center.X() - const.HUB_SIDE / 2,
        ...
        def fuel_hit_net(self, fuel: "FuelSim.Fuel") -> float:
            ...
            """
            if (
                fuel.pos.Z() > const.HUB_NET_HEIGHT_MAX
                or fuel.pos.Z() < const.HUB_NET_HEIGHT_MIN
            ):
                return 0
            if (
                fuel.pos.Y() > self.center.Y() + const.HUB_NET_WIDTH / 2
            ...
            if (
                fuel.pos.X()
                > self.center.X() + const.HUB_NET_OFFSET * self.exit_vel_x_mult
            ):
                return max(
                    0,
                    self.center.X()
                    + const.HUB_NET_OFFSET * self.exit_vel_x_mult
                    - (fuel.pos.X() - const.FUEL_RADIUS),
                )
            else:
                return min(
                    0,
    ...
    class SimIntake:
        ...
        def should_intake(self, fuel: "FuelSim.Fuel", robot_pose: Pose2d) -> bool:
            ...
            """
            if (
                not self.able_to_intake() or fuel.pos.Z() > 0.3
            ):  # Assuming default bumper height
                return False
            ...
            result = (
                self.x_min <= fuel_relative_pos.X() <= self.x_max
